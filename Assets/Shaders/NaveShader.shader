...
Shader "Adal/NaveShader" {
    Properties {
        ...
        _Normal_Intensity ("Normal_Intensity", Range(0, 1)) = 0.5
        ...
        _Anisotropic_Direction ("Anisotropic_Direction", Range(0, 1)) = 0.4108801
        _Anisotropic_Power ("Anisotropic_Power", Range(0, 1)) = 0.2925076
        _Anisotropic_Colro ("Anisotropic_Colro", Color) = (0.5,0.5,0.5,1)
        _Anisotropic_Intensity ("Anisotropic_Intensity", Range(0, 1)) = 0.5037508
        ...
        _Color_Efecto ("Color_Efecto", Color) = (0.5700995,0.1933962,0.7735849,1)
...
////// Emissive:
                ...
                float node_5401 = (attenuation*max(0,dot(lightDirection,normalDirection)));
                float3 Light_Data = (node_5401*_LightColor0.rgb);
                float3 node_7898 = Light_Data;
                float3 Oscuro_Mas_Oscuro = saturate((1.0-(1.0-(_Color_Efecto.rgb*(1.0 - Height_Gradient)*0.5))*(1.0-(UNITY_LIGHTMODEL_AMBIENT.rgb*Diffuse*node_7898))));
                float3 emissive = Oscuro_Mas_Oscuro;
                float Shadow_Data = node_5401;
                float3 Specular = (_Specular_Color.rgb*(pow(max(0,dot(normalDirection,halfDirection)),exp(lerp(0,10,_Specular_Power)))*exp(lerp(0,10,_Specular_Intensity)))*Shadow_Data);
                float3 Anisotropic_Data = ((lerp(0,2,_Anisotropic_Intensity)*pow((1.0 - abs(dot(halfDirection,lerp(i.bitangentDir,i.tangentDir,_Anisotropic_Direction)))),exp(lerp(0,10,_Anisotropic_Power)))*_Anisotropic_Colro.rgb)*Shadow_Data);
                ...
                float3 Refraction_Info = tex2D( _GrabTexture, lerp( (sceneUVs.rg+(_NoiseMap_Refraction_var.r*_Refraction_Intesity)), (sceneUVs.rg+(_Refraction_Intesity*float2(_Normal_Refrection_var.r,_Normal_Refrection_var.g))), _Use_Normal_Refraction )).rgb;
                float node_6246 = Height_Gradient;
                float3 Producto_Final = saturate((1.0-(1.0-(saturate((1.0-(1.0-(node_7898*Efecto_Shingon))*(1.0-Fresnel_Data)))*Shadow_Data))*(1.0-(Refraction_Info*(1.0 - node_6246)))));
