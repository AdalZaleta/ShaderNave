...
Shader "Adal/Shader2Text" {
    Properties {
        ...
        _Textura_2 ("Textura_2", 2D) = "white" {}
        [MaterialToggle] _Use_Specular_Map ("Use_Specular_Map", Float ) = 0
        _Normal_Intesity ("Normal_Intesity", Range(0.1, 2)) = 1.359251
        _Color_Pantalones_Sombra ("Color_Pantalones_Sombra", Color) = (0.3396226,0.2885952,0.232289,1)
        _Color_Placa_Cinturon_Sombra ("Color_Placa_Cinturon_Sombra", Color) = (0.3490566,0.2548658,0.2189836,1)
        _Color_Camisas_sombra ("Color_Camisas_sombra", Color) = (0.5283019,0.2815949,0.3218735,1)
        _Color_Piel_Sombra ("Color_Piel_Sombra", Color) = (0.8584906,0.7146459,0.62767,1)
        _Color_Chaleco_Sombra ("Color_Chaleco_Sombra", Color) = (0.1415094,0.1401744,0.1381719,1)
        _Color_Cintas_Sombra ("Color_Cintas_Sombra", Color) = (0.8584906,0.8584906,0.8584906,1)
    }
    SubShader {
        Tags {
            "RenderType"="Opaque"
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                o.pos = UnityObjectToClipPos( float4(v.vertex.xyz + v.normal*(_Outline_Width*0.05+0.0),1) );
                return o;
            }
        ...
        Pass {
            ...
            uniform sampler2D _Textura_2; uniform float4 _Textura_2_ST;
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float4 _Textura_2_var = tex2D(_Textura_2,TRANSFORM_TEX(i.uv0, _Textura_2));
                float3 NormalData = (normalize(((float3(_Textura_2_var.rgb.rg,0.0)+float3(0,0,1))*2.0+-1.0))*_Normal_Intesity);
...
////// Emissive:
                ...
                float node_451_if_leA = step(node_6191.r,0.95);
                float node_451_if_leB = step(0.95,node_6191.r);
                float node_119 = 0.0;
                float node_3527 = 1.0;
                ...
                float Specular_Data = _Textura_2_var.b;
                ...
                float3 finalColor = emissive + ((lerp((node_4611_if_leA*node_1486)+(node_4611_if_leB*(node_8814*Custom_Light_Data)),node_1486,node_4611_if_leA*node_4611_if_leB)*lerp( 1.0, Ambiet_Oclusion_Data, _Use_AO ))+Specular);
...
////// Lighting:
                ...
                float Shadow_Data = node_3001;
                float3 Specular = ((_Specular_Color.rgb*pow(max(0,dot(i.normalDir,halfDirection)),exp(_Specular_Power))*_Specular_Intensity*lerp( 1.0, Specular_Data, _Use_Specular_Map ))*Shadow_Data);
                float3 finalColor = ((lerp((node_4611_if_leA*node_1486)+(node_4611_if_leB*(node_8814*Custom_Light_Data)),node_1486,node_4611_if_leA*node_4611_if_leB)*lerp( 1.0, Ambiet_Oclusion_Data, _Use_AO ))+Specular);
